...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\nPor favor seja ativo otaku fedido😃`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Editando calma ⌛',
				success: '✔ FOI AINDA BEM ✔️',
				error: {
					stick: '⚠️ Falha ao fazer a figurinha por favor tente dnv⚠️',
					Iv: '❌ Link invalido ❌'
				},
				only: {
					group: '❌ Este comando só pode ser usado em grupos! ❌',
					ownerG: '⚠️ Este comando só pode ser usado pelo CAFFEZINMDZZ! 😂',
					ownerB: '❌ Este comando só pode ser usado pelo CAFFEZINMDZZ ❌',
					admin: '⚠️ Este comando só pode ser usado pelo o adm! 😝',
					Badmin: '❌ Esse comando so vai ser usado quando eu for adm ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["556198302738@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falha ao transforma ${tipe} a figurinha`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
				...
				case 'porno':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Adm proibiu isso no grupo🙄'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/d1M6lOz.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* CAFFEZINMDZZ\n\n*WPP:* wa.me/556198302738\n*INSTA:* to sem aff kk\n\n\n*Digite *.bot* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'mia2':
					...
					client.sendMessage(from, buffer, video, {quoted: mek, caption: 'use drogaskk'})
					break
				case 'memeindo':
					memein = await kagApi.memeindo()
				...
				case 'limpar':
					...
					break
				case 'ts':
					if (!isOwner) return reply('Quem é você man?')
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse otaku aqui @${mentioned[0].split('@')[0]} agora é admin então respeitem ok?! 😂`, mentioned, true)
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar a pessoa tente novamente')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
